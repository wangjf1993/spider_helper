...
def check_and_start(host, source_dir, spider_name, spider_cmd):
    if not check_pass(host, spider_name, spider_cmd.get("command")):
        start_spider(host, source_dir, spider_cmd.get("subdir"), spider_cmd.get("command"))
        dprint("......spider %s started." % spider_name)
        dprint("......command: %s" % spider_cmd.get("command"))
